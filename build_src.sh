...
if [ -z "$FORGEJO_VERSION" ] || [ -z "$BUILD_VERSION" ]; then
    echo "Usage: $0 <forgejo_version> <build_version>"
    echo "Example: $0 15.0.2 1"
    exit 1
fi

PACKAGE_NAME="forgejo"
ORIG_TARBALL="${PACKAGE_NAME}_${FORGEJO_VERSION}.orig.tar.gz"
BUILD_DIR="${PACKAGE_NAME}-${FORGEJO_VERSION}"

echo "Creating Debian/Ubuntu source packages for forgejo ${FORGEJO_VERSION}-${BUILD_VERSION}..."
...
if [ ! -f "$ORIG_TARBALL" ]; then
    ...
    wget -q "https://github.com/forgejo/forgejo/archive/refs/tags/v${FORGEJO_VERSION}.tar.gz" -O "$ORIG_TARBALL"
...
build_source_package() {
    local dist=$1
    local FULL_VERSION="${FORGEJO_VERSION}-${BUILD_VERSION}~${dist}"
    ...
    rm -rf "$BUILD_DIR"
    tar -xf "$ORIG_TARBALL"
    # GitHub archives extract as forgejo-x.y.z/ which matches our BUILD_DIR
...
forgejo (${FULL_VERSION}) ${dist}; urgency=medium

  * New upstream release ${FORGEJO_VERSION}.
